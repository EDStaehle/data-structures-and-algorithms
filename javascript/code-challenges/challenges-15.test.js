...
const screenForNames = (arr) => {
  return arr.filter(str => /^[A-Z]{1}\w{1,2}\.\s[^\s]\w+/.test(str));
}
...
let biggerThanLuke = (arr) => {
  const newArr = [];
  arr.forEach(obj => obj.mass > +starWarsData[0].mass ? newArr.push(obj.name) : null);
  return newArr.join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  let resultsHorizontal = board.map(row => row);
  let resultsVert = [];
  for(let i = 0; i < 3;i++){
    resultsVert.push(board.map(item => item[i]));
  }
  let resultsDiagLR = board.map((row, idx) => row[idx]);
  let resultsDiagRL = board.reverse().map((row, idx) => row[idx]);
  let results = [...resultsHorizontal, ...resultsVert, resultsDiagLR, resultsDiagRL];
  if(results.map(arr => ticTacHelper(arr)).includes(true)){
    return true;
  }else{return false;}
};
